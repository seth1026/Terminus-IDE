...
const BugReports = () => {
    ...
    useEffect(() => {
        const fetchBugReports = async () => {
            try {
                const response = await fetch(`${import.meta.env.VITE_API_URL}/admin/getAllBugReports`, {
                    method: "GET",
                    headers: {
...
const BugReportTable = ({ token, bugReports, refreshTrigger, setBugReports, setRefreshTrigger, setPopupVisible, setPopupMessage, setPopupType, popupMessage, popupType, popupVisible }) => {
    const handleDeleteBugReport = async (id) => {
        try {
            const responce = await fetch(`${import.meta.env.VITE_API_URL}/admin/deleteBugReport`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token.token}`
                            ...
                            <tr key={bugReport._id} className="hover:bg-gray-50/60 transition-all duration-200">
                                ...
                                <td className="px-6 py-4">
                                    <button
                                        onClick={async () => {
                                            try {
                                                const response = await fetch(`${import.meta.env.VITE_API_URL}/admin/toggleBugReportSeen`, {
                                                    method: "POST",
                                                    headers: {
                                                        "Content-Type": "application/json",
                                                        "Authorization": `Bearer ${token.token}`
